...
### Что это?
**Zitadel** - микросервис авторизации и управления пользователями (Identity and Access Management).
...
| **RAM (request)** | 512Mi |
| **RAM (limit)** | 1Gi |
| **Storage** | Использует внешнюю PostgreSQL (VM Database) |
| **Сеть** | ClusterIP Service + Ingress (https://auth.autolabs.local) |

### Зависимости
- **PostgreSQL** (VM Database) - хранение пользователей, ролей, сессий
- **Nginx Ingress** - внешний доступ для OAuth2 redirect flow
...
**Проблема без централизованной авторизации:**
- Backend Platform должен самостоятельно управлять пользователями (регистрация, пароли, сессии)
- При добавлении новых сервисов - дублирование логики авторизации
- Сложность реализации OAuth2/OIDC для Frontend
- Отсутствие единого места управления правами доступа

**Решение - Zitadel:**
1. **Single Sign-On (SSO)** - один раз логин, доступ ко всем сервисам платформы
2. **Централизованное управление** - все пользователи, роли, пермишаны в одном месте
3. **Стандартные протоколы** - OAuth2/OIDC (не нужно изобретать велосипед)
4. **Безопасность** - проверенное решение с регулярными security updates
5. **Масштабируемость** - готовность к добавлению новых сервисов (будущие микросервисы)

### Вклад в работу платформы

**Zitadel обеспечивает:**
- ✅ **Аутентификацию** - проверка "кто этот пользователь?" (логин/пароль)
- ✅ **Авторизацию** - проверка "что может делать этот пользователь?" (роли, пермишаны)
- ✅ **Управление сессиями** - токены, refresh, logout
- ✅ **Аудит** - логирование всех действий пользователей (кто, когда, что делал)

**Без Zitadel:**
- Backend Platform разрастается (добавляется логика user management)
- Дублирование кода при добавлении новых сервисов
- Сложность поддержки безопасности

**С Zitadel:**
- Backend Platform фокусируется на бизнес-логике (курсы, лабы)
- Новые сервисы просто интегрируются через OAuth2
- Централизованная безопасность

---

## Обоснование выбора Zitadel

### Почему Zitadel, а не альтернативы?

**Альтернативы:**
- **Keycloak** (Red Hat) - более зрелый, но тяжёлый (Java, высокое потребление ресурсов)
- **Auth0** (облачный) - платный, зависимость от внешнего сервиса
- **Ory Hydra** - сложнее в настройке, меньше features "из коробки"
- **Самописная авторизация** - высокий риск уязвимостей, долгая разработка

**Преимущества Zitadel:**
1. **Современность** - CNCF проект, активное развитие
2. **Легковесность** - написан на Go (низкое потребление ресурсов vs Keycloak на Java)
3. **Cloud-native** - идеально для Kubernetes
4. **Rich UI** - удобная админ-панель для управления пользователями
5. **Multi-tenancy** - готовность к развитию платформы (несколько организаций)
6. **OpenID Certified** - соответствие стандартам

**Требования AutoLabs → Zitadel:**

| Требование | Zitadel | Keycloak | Auth0 |
|------------|---------|----------|-------|
| OAuth2/OIDC | ✅ | ✅ | ✅ |
| Низкие ресурсы | ✅ (Go) | ❌ (Java) | N/A (облако) |
| Self-hosted | ✅ | ✅ | ❌ |
| Open-source | ✅ | ✅ | ❌ |
| RBAC + ABAC | ✅ | ✅ | ✅ (платно) |
| Простота настройки | ✅ | ⚠️ | ✅ |

**Вывод:** Zitadel - оптимальный баланс функциональности, производительности и простоты для AutoLabs.
